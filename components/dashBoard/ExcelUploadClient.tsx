...
export default function ExcelUploadClient() {
    ...
    const handleUpload = async () => {
        ...
        try {
            ...
            if (upResult && upResult.inserted >= -1) {
                ...
                toast.custom((t) => (
                    <ToastUI t={t} message="데이터 보내기 성공!!" />
                ), {
                    duration: 2000,
                    onAutoClose: () => {
                        router.push('/');
            ...
            toast.custom((t) => (
                <ErrorToastUI t={t} message={(error as Error).message || "알 수 없는 오류가 발생했습니다."} />
            ), {
    ...
    return (
        <div className="space-y-8">
            ...
            {/* 조건부 렌더링: showToast가 true일 때만 화면에 나타남 */}
            {/* {showToast && <SuccessToast message="데이터 보내기 성공!!" />} */}
        </div>
    );
}